...
//		SX1278_setACK(module, packet, ack);
...
int SX1278_LoRaTxResendPacket(SX1278_t *module, SX1278_pack_t *packet_sent, uint8_t *txBuffer, uint8_t length,
		uint32_t timeout, uint8_t dest) {
//	SX1278_setPacket(module, dest, (uint8_t*)txBuffer, packet_sent);
	SX1278_SPIWrite(module, LR_RegOpMode, 0x8b);	//Tx Mode
	while (1) {
...
void SX1278_getPacket(SX1278_t *module, SX1278_pack_t *packet_received)
{
	...
	packet_received->src = SX1278_hw_SPIReadByte(module->hw);
	packet_received->packnum = SX1278_hw_SPIReadByte(module->hw);
	packet_received->length = SX1278_hw_SPIReadByte(module->hw);
	packet_received->retry = SX1278_hw_SPIReadByte(module->hw);
...
void SX1278_SendRetries(SX1278_t *module, SX1278_pack_t *packet, uint8_t *txBuf,uint8_t dest){
	uint8_t ret;
	if (0 <=packet->retry && packet->retry < 3){
		ret = SX1278_transmit(module, packet, txBuf, 255, 2000, dest);
		SX1278_hw_DelayMs(1);
	}
}
